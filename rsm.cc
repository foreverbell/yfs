...
// is:
//    - inviewchange: a node has failed and the RSM is performing a
//                    view change
...
rsm::recovery()
{
  ...
  while (1) {
    ...
    if (primary == cfg->myaddr()) {
      tprintf("recovery: sync vid_insync %d, i am primary\n", vid_insync);
      r = sync_with_backups();
    } else {
      tprintf("recovery: sync vid_insync %d, i am backup\n", vid_insync);
      r = sync_with_primary();
    }
...
rsm::sync_with_backups()
{
  ...
  tprintf("sync_with_backups: insync is true now\n");
  insync = true;
...
rsm::statetransferdone(std::string m)
{
  ...
  tprintf("rsm::statetransferdone: %s success\n", m.c_str());
...
rsm::join(std::string m)
{
  ...
  rpcc *cl = h.safebind();
  if (cl != 0) {
    ret = cl->call(rsm_protocol::joinreq, cfg->myaddr(), last_myvs, r, rpcc::to(120000));
  }
  VERIFY(pthread_mutex_lock(&rsm_mutex) == 0);
...
rsm::client_invoke(int procno, std::string req, std::string &r)
{
  ...
  tprintf("client_invoke of procno = %d.\n", procno);

  pthread_mutex_lock(&rsm_mutex);

  if (inviewchange) {
    tprintf("client_invoke: in a view change.\n");
    pthread_mutex_unlock(&rsm_mutex);
    return rsm_client_protocol::BUSY;
  }

  if (primary != cfg->myaddr()) {
    tprintf("client_invoke: not a primary.\n");
    pthread_mutex_unlock(&rsm_mutex);
...

  {
    ...
    // FIXME (fb): execute should be protected in rsm_mutex.
    // Consider refactoring execute by passing procs[procno] instead.
...
//
rsm_protocol::status
rsm::invoke(int proc, viewstamp vs, std::string req, int &)
{
  ScopedLock ml(&rsm_mutex);
  std::string r;

  if (vs != myvs) {
    tprintf("invoke: not expected viewstamp.\n");
    return rsm_protocol::ERR;
  }

  if (inviewchange) {
    tprintf("invoke: in a view change.\n");
    return rsm_protocol::BUSY;
  }

  if (!cfg->ismember(cfg->myaddr(), vs.vid)) {
    tprintf("invoke: not a member.\n");
    return rsm_protocol::ERR;
  }
                 ...
                 rsm_protocol::transferres &r)
{
  ...
  tprintf("transferreq from %s (%d,%d) vs (%d,%d), insync %d\n", src.c_str(),
          last.vid, last.seqno, last_myvs.vid, last_myvs.seqno, (int) insync);
...
// RPC handler: Inform the local node (the primary) that node m
// has synchronized.
rsm_protocol::status
rsm::transferdonereq(std::string m, unsigned vid, int &)
{
  ...
  // 1. Return BUSY if I am not insync, or if the slave is not synchronizing
  //    for the same view with me.
  // 2. Remove the slave from the list of unsynchronized backups.
  // 3. Wake up recovery thread if all backups are synchronized.
...
// so the client can switch to a different primary when it existing primary
// fails.
rsm_client_protocol::status
rsm::client_members(int i, std::vector<std::string> &r)
{
  ...
  std::vector<std::string> m = cfg->get_view(vid_commit);
  m.push_back(primary);
  r = m;
